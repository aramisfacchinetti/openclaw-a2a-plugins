...
import {
  A2A_OUTBOUND_CONFIG_JSON_SCHEMA,
...
test('manifest configSchema stays in lockstep with TypeScript JSON schema export', () => {
  ...
  assert.deepEqual(manifestConfigSchema, A2A_OUTBOUND_CONFIG_JSON_SCHEMA)
})

test('OpenClaw configSchema export reuses the canonical JSON schema', () => {
  assert.deepEqual(
    A2A_OUTBOUND_OPENCLAW_CONFIG_SCHEMA.jsonSchema,
    A2A_OUTBOUND_CONFIG_JSON_SCHEMA,
  )
})
