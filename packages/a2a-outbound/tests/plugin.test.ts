...
function readStructuredContent(result: unknown): A2AToolResult {
  const toolResult = asRecord(result) as ToolResultLike

  if (toolResult.structuredContent !== undefined) {
    return toolResult.structuredContent as A2AToolResult
  }
  ...
  return JSON.parse(firstRecord.text) as A2AToolResult
}
...
test('plugin registers the three a2a tools with optional flag', () => {
  const tools: RegisteredTool[] = []
